...
- **左扰动位姿优化 (Left-Perturbation Optimization)**：
  - 基于高斯-牛顿法（Gauss-Newton）直接在 $\text{SE}(3)$ 流形上进行迭代求解。
  - 精确实现了解析雅可比矩阵 $J = [I, -[x]_\times]$ 的构建与信息矩阵（Hessian）的累加。
...
- 反对称矩阵（Skew-symmetric matrix）: $\mathbf{v}^\wedge$
- $\text{SE}(3)$ 指数映射: $\exp(\boldsymbol{\xi}^\wedge) = \begin{bmatrix} \exp(\boldsymbol{\phi}^\wedge) & \mathbf{J}_l \boldsymbol{\rho} \\ \mathbf{0}^T & 1 \end{bmatrix}$
- 误差状态更新（左扰动）: $\mathbf{T} \leftarrow \exp(\delta\boldsymbol{\xi}^\wedge) \cdot \mathbf{T}$
